...
public class Spline extends Path {
	...
	private List<BezierCurve> createBezierCurves(PathData startPathData, List<List<Pose>> pathControlPoints) {
		double nextStartVelocity;
		double nextEndVelocity;
		PathData nextStartPathData = startPathData;
		ListIterator<List<Pose>> iterator = pathControlPoints.listIterator();
		...
		while (iterator.hasNext()) {
			...
			BezierCurve curve = new BezierCurve(getVCruise(), getAMax(), nextStartVelocity, nextEndVelocity,
				isBackwards(),
				nextStartPathData,
				iterator.next());
