...
//			}).run();
  ...
  public static void printPath(Path path) {
    ...
    for (PathData aPathData : pathData) {
      ...
      System.out
          .println(aPathData.getCenterPose().getX() + " " + aPathData.getCenterPose().getY() + ' '
              + Math.toDegrees(aPathData.getCenterPose().getAngle()) + " t: " + aPathData
              .getTime());

      System.out.println(
          aPathData.getLeftState().getLength() + " " + aPathData.getRightState().getLength() + ' '
              + aPathData.getLeftState().getVelocity() + ' ' + aPathData.getRightState()
              .getVelocity());
...
//
    ...
    for (int i = 0; i <= 100; i++) {
      Pose pose = dynamicBezierCurve.createPathData(new PathData(new Pose(0, 0)), i / 100.0)
          .getCenterPose();
